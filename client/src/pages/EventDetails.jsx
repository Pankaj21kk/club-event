...
const EventDetails = () => {
  ...
  const fillPct  = Math.min(100, Math.round((registeredCount / totalSeats) * 100));
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-0 border-2 border-black rounded-sm overflow-hidden mb-10">
              {[
                ...
                },
                {
                  icon: 'ri-group-line',
                  label: 'Seats',
                  value: `${registeredCount} / ${totalSeats} Registered`,
                },
              ].filter(Boolean).map((meta, i) => (
                <div
                  key={i}
                  className={`flex items-start gap-4 p-5 ${i % 2 === 0 ? 'border-r border-black' : ''} ${i < 2 ? 'border-b-2 border-black' : ''}`}
                >
